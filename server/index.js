...
// 兼容 OpenRouter 的 /v1/chat/completions
  ...
  try {
    ...
    if (!agents || agents.length === 0) {
      ...
      return res.json({
        ...
        choices: [{
          ...
          message: {
            ...
            content: JSON.stringify({
              ...
              message: '任务已接收，请通过 AgentCore 任务市场执行',
              task: lastMessage.substring(0, 100)
            })
          },
          finish_reason: 'stop'
        }],
        usage: {
          ...
          total_tokens: lastMessage.length + 50
        }
      });
    }
    ...
    res.json({
      ...
      usage: {
        ...
        total_tokens: lastMessage.length + 100
      }
    });
  } catch (error) {
    res.status(400).json({
